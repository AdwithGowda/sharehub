...
export default function EditItem() {
  ...
  useEffect(() => {
    if (itemImages.length === 0) {
      if (existingImageUrl) {
        setImagePreviews([existingImageUrl]);
      } else {
        setImagePreviews([]);
      }
      return;
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setCategory(itemData.category || itemData.category_id || '');
        setLocation(itemData.location || '');
        setPricePerDay(itemData.price_per_day || '');
        setDepositAmount(itemData.deposit_amount || '');
        setDescription(itemData.description || '');
        
        if (itemData.images && itemData.images.length > 0) {
          const imgUrl = itemData.images[0].image.startsWith('http') 
            ? itemData.images[0].image 
            : `http://localhost:8000${itemData.images[0].image}`;
          setExistingImageUrl(imgUrl);
        }
      } catch (err) {
        setError('Could not load item details. Please refresh the page.');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user?.is_verified) {
      setError('Complete KYC verification before editing an item.');
      return;
    }
    ...
    try {
      ...
      setSuccess('Your item listing has been updated successfully.');
      setTimeout(() => navigate(`/item/${id}`), 1200);
    } catch (err) {
      const data = err.response?.data;
      const apiError = data?.error || data?.detail || Object.values(data || {}).flat().join(' ');
      setError(apiError || 'A problem occurred while updating your listing.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-xs animate-fade-in">
      <div className="mb-6">
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Edit Gear Listing</h2>
        <p className="text-sm text-slate-500 mt-1">Update your item's details for the marketplace.</p>
      </div>
      ...
      {!user?.is_verified && (
        <div className="mb-5 p-4 bg-amber-50 text-amber-800 rounded-2xl text-xs font-semibold border border-amber-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <span>Your account must be KYC verified before you can update rental listings.</span>
          <Link
            to="/dashboard/kyc"
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Item Image (Optional: Upload new to replace)</label>
